      ...
      <div className="min-h-screen bg-[#000000] flex items-center justify-center p-6 text-[#1DCD9F] font-mono">
        <div className="max-w-2xl w-full border-4 border-[#1DCD9F] p-10 rounded-lg bg-[#111] shadow-[0_0_50px_rgba(29,205,159,0.2)]">
          <div className="flex items-center gap-4 mb-8">
            ...
            <h1 className="text-3xl font-black uppercase tracking-tighter">System Integration Required</h1>
          </div>
          
          <p className="text-sm leading-loose mb-8 opacity-80 uppercase tracking-wide">
            The Neural Architect is offline. No valid API credentials detected in the environment. 
          </p>

          <div className="space-y-6">
            <section>
              <h2 className="text-xs font-black uppercase tracking-widest border-b border-[#1DCD9F]/30 pb-2 mb-4">Action Sequence for Vercel:</h2>
              <ol className="list-decimal list-inside space-y-3 text-xs opacity-70">
                <li>Go to your project dashboard at <a href="https://vercel.com" className="underline hover:text-white">vercel.com</a></li>
                <li>Navigate to <span className="text-white">Settings &gt; Environment Variables</span></li>
                <li>Add a new Key: <span className="bg-[#1DCD9F] text-[#000] px-2 font-bold">API_KEY</span></li>
                <li>Paste your Gemini API Key into the Value field</li>
                <li>Go to <span className="text-white">Deployments</span> and "Redeploy" your latest commit</li>
              </ol>
            </section>
            
            <div className="pt-6 border-t border-[#1DCD9F]/30">
              <p className="text-[10px] uppercase opacity-40">Status: Terminal Awaiting Signal...</p>
              <button 
                onClick={() => window.location.reload()} 
                className="mt-4 px-6 py-2 border-2 border-[#1DCD9F] hover:bg-[#1DCD9F] hover:text-[#000] transition-all font-black text-xs uppercase"
              >
                Retry Connection
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-5 space-y-8">
            <section className="bg-[#222222] border-2 border-[#1DCD9F] rounded-lg p-6 shadow-[8px_8px_0px_0px_rgba(22,153,118,1)]">
              ...
              <div className="mb-8">
                ...
                {media ? (
                  <div className="relative group rounded-lg overflow-hidden border-2 border-[#1DCD9F]">
                    ...
                    <button onClick={reset} className="absolute top-2 right-2 bg-red-600 text-white p-2 rounded hover:bg-red-500 shadow-lg">✕</button>
                  ...
                  <div onClick={() => fileInputRef.current?.click()} className="border-4 border-dotted border-[#169976]/50 hover:border-[#1DCD9F] rounded-lg p-10 flex flex-col items-center justify-center gap-4 cursor-pointer group transition-all bg-[#000000]">
              ...
              {error && <div className="mb-6 p-4 bg-red-900/20 border-2 border-red-700 rounded-lg text-red-500 text-xs font-bold uppercase">{error}</div>}
          ...
          <div className="lg:col-span-7">
            {isGenerating ? (
                ...
                <div className="bg-[#000000] border-2 border-[#1DCD9F] rounded-lg overflow-hidden shadow-[8px_8px_0px_0px_rgba(29,205,159,1)]">
                  <div className="bg-[#1DCD9F] px-6 py-4 border-b-2 border-[#000000] flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleCopy(result.optimizedPrompt, 'PROMPT')} className="text-[10px] bg-[#000000] text-[#1DCD9F] px-3 py-1.5 rounded font-black uppercase border border-[#000000]">Copy Plain</button>
                      <button onClick={() => handleCopy(`/imagine prompt: ${result.optimizedPrompt} --v 6.1 --stylize 250`, 'MIDJOURNEY')} className="text-[10px] bg-[#222222] text-[#1DCD9F] px-3 py-1.5 rounded font-black uppercase border border-[#1DCD9F]/30">Midjourney Format</button>
                    </div>
                  </div>
              ...
              <div className="h-full flex flex-col items-center justify-center p-12 bg-[#222222]/30 border-4 border-[#169976] border-dotted rounded-lg text-[#169976]">
                ...
                <p className="text-[10px] mt-2 font-bold uppercase opacity-50 text-center">Terminal awaiting sequence ingestion. Paste or upload data to begin deconstruction.</p>
              </div>
            )}
          </div>
        </div>
